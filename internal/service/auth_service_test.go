...
func Test_authService_Attempt(t *testing.T) {
	...
	t.Run("test success attempt user", func(t *testing.T) {
		userRepository.Mock.On("FindByEmail", "agra@user.com").Return(&model.User{ID: 1, Name: "agra", Email: "agra@user.com", Password: "$2a$04$93AZUXoqhOu6TNb481MYke3iDbM8UAzizOHmKSEf36bQtzV3kffwm"}, nil).Once()
...
func Test_authService_ResetPass(t *testing.T) {
	...
	t.Run("test success reset password", func(t *testing.T) {
		user := model.User{ID: 1, Name: "agra", Email: "agra@user.com", Password: "12345"}
